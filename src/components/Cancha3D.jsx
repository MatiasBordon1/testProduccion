...
function Scene({
  ...
  // Genera los lotes con memoización
  const lotes = useMemo(
    () =>
      crearLotes({
        colorBase: getColor,
  ...
  return (
    ...
    >
      ...
      {/* Piso invisible para recibir la sombra */}
      <mesh
        rotation={[-Math.PI / 2, 0, 0]}
        position={[0, -4, 0]} // debajo de la base azul
        receiveShadow
      >
...
}, [!!effectiveAutoRotate, !!effectiveTopView, contactOpen]); // 👈 agregado contactOpen

  const getTierOf = (id) => {
    if (oroLotes.includes(id)) return 'oro';
    if (plataLotes.includes(id)) return 'plata';
    return 'bronce';
  };
